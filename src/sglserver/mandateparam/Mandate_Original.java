...
public class Mandate_Original {
	 ...
	 */
	public Mandate_Original(String filename){
		try{
			readXML(filename);
		}catch(Exception e){
			e.printStackTrace();
		...
		for(int i=0;i<pe.getLength();i++){
			...
			if(pe.item(i).getParentNode().getNodeName().equals("KeyAgreement")){
				...
				for(int j=0;j<ro.getLength();j++){
					...
					// Option要素のノードの値を取得
					String option = op.item(i).getFirstChild().getNodeValue();
					...
					String groupname;
					if(behavior.equals("Exchange"))
						groupname = gn.item(0).getFirstChild().getNodeValue();
					else
						groupname = "red";
					// 前後の空白を取り除く
